...
from keras.layers import Dense, Dropout, Embedding, LSTM, Conv1D, MaxPooling1D, Bidirectional
...
def runTraining(X_train, y_train, X_validation, y_validation, embedding_matrix,
    ...
    # separated layers
    y1 = Conv1D(filters=32, kernel_size=3, padding='same', activation='relu')(main_branch)
    y1 = MaxPooling1D(pool_size=2)(y1)
    y1 = Bidirectional(LSTM(num_neurons))(y1)
    y1 = Dense(1, activation='softmax', name='age')(y1)

    y2 = Conv1D(filters=32, kernel_size=3, padding='same', activation='relu')(main_branch)
    y2 = MaxPooling1D(pool_size=2)(y2)
    y2 = Bidirectional(LSTM(num_neurons))(y2)
